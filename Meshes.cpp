...
void Meshes::load(std::string const &filename, Attributes const &attributes) {
    ...
    { //read + upload data chunk:
        ...
        if (attributes.Colour != -1U) {
            glVertexAttribPointer(attributes.Colour, 3, GL_FLOAT, GL_FALSE, sizeof(v3n3), (GLbyte *)0 + sizeof(glm::vec3));
            glEnableVertexAttribArray(attributes.Colour);
        }
        else {
            std::cerr << "WARNING: loading v3n3 data from '" << filename << "', but not using the Colour attribute." << std::endl;
        }
    }
